    ...
    private static void mergeMap(Map input, String prefix, ArrayList<String> output) {
        ...
        for (Map.Entry entry : entries) {
            ...
            if (entry.getValue() instanceof Map)
                mergeMap((Map) entry.getValue(), prefix + entry.getKey() + ".", output);
            else if (entry.getValue() instanceof List)
                mergeList((List) entry.getValue(), prefix + entry.getKey() + ".", output);
            else
                output.add(prefix + entry.getKey() + "=" + entry.getValue());
        }
    }
